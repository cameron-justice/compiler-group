...
namespace semantics
{
	...
	const types::Type* TypeChecking::visit(const SimpleVar* v)
	{
		...
		//if variable is defined return, else it's undefined
	...
	const types::Type* TypeChecking::visit(const SubscriptVar* v)
	{
		...
		visit(v->getIndex());								//visit the index
		return visit(v->getVar());							//visit and return
	}

	const types::Type* TypeChecking::visit(const OpExp* e)
	{
		/* check both operands */
		const types::Type* t1 = visit(e->getLeft());		//visit left, store in t1
		const types::Type* t2 = visit(e->getRight());		//visit right, store in t2

		//if types are equivalent return t1, else there is a semantic error because its not equivalent
		if (t1 == t2) {												
			return t1;
		}
		else {
	...
	{
		/* check the variable */
		return visit(e->getVar());						//visit variable and return
	...
	const types::Type* TypeChecking::visit(const CallExp* e)
	{
		...
		*/
		...
		//if the function name is defined check every argument and return, else the function name is undefined
	...
	const types::Type* TypeChecking::visit(const SeqExp* e)
	{
		...
		//while list is not null, visit every expression in sequence and return
		const types::Type* t = NULL;
	...
	//This function checks both variable and expression
	const types::Type* TypeChecking::visit(const AssignExp* e)
	{
		/* check both variable and expression */
		visit(e->getVar());								//visit variable
		return visit(e->getExp());						//visit expression and return

	}

	//This function checks test condition, then-clause, and else-clause
	const types::Type* TypeChecking::visit(const IfExp* e) {
		/* check test condition, then-clause, and else-clause (if exists) */

		//if test condition is not null, return
		const types::Type* t = visit(e->getTest());
		if (e->getTest() != NULL) {
			return t;
		}

		//if then-clause is not null, return
		const types::Type* t1 = visit(e->getThenClause());
		if (e->getThenClause() != NULL) {
			return t1;
		}

		//if else-clause is not null, return
	...
	const types::Type* TypeChecking::visit(const WhileExp* e)
	{
		...
		//visit whileexp, if not null then return
		const types::Type* t = visit(e->getTest());
		if (e->getTest() != NULL) {
			return t;
		}
	...
	//This function checks for break expression
	const types::Type* TypeChecking::visit(const BreakExp* e)
	{
		return NULL;
	}

	//This function checks let expressions
	...
	//This function checks for array declarations
	const types::Type* TypeChecking::visit(const ArrayExp* e)
	{
		...
		*/
		...
		//if array is in symbol table visit size and init then return init, else array type is undefined
	...
	const types::Type* TypeChecking::visit(const VarDec* d) {
		...
		*/
		...
		//insert varible to symbol table
		insertVar(d->getName(), symbol::SymTabEntry(env.getVarEnv()->getLevel(), NULL, d));

		//if type is provided visit type
		if (d->getType())
			visit(d->getType());

		//visit init and return
	...
	const types::Type* TypeChecking::visit(const TypeDec* d)
	{
		...
		*/
		...
		//insert name of new type to symbol table and visit/return type info
	...
	const types::Type* TypeChecking::visit(const NameTy* t)
	{
		...
		*/
		...
		//if type name is defined return, else type is undefined
	...
	const types::Type* TypeChecking::visit(const ArrayTy* t)
	{
		...
		*/
		...
		//if array type is in symbol table return, else array type is undefined
		if (env.getTypeEnv()->contains(t->getName())) {
